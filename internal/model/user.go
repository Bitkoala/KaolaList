...
func (u *User) ValidatePwdStaticHash(pwdStaticHash string) error {
	...
	// Try Bcrypt first
	err := bcrypt.CompareHashAndPassword([]byte(u.PwdHash), []byte(pwdStaticHash))
	if err == nil {
		return nil
	}

	// Fallback to legacy SHA256 if it's not a bcrypt hash
	if u.PwdHash == HashPwd(pwdStaticHash, u.Salt) {
		// Migration: Upgrade to bcrypt on next successful login if possible
		// (This logic might be better placed in the auth handler, but here we confirm it's valid)
		return nil
	}
...
func (u *User) SetPassword(pwd string) *User {
	...
	hash, _ := bcrypt.GenerateFromPassword([]byte(StaticHash(pwd)), bcrypt.DefaultCost)
	u.PwdHash = string(hash)
