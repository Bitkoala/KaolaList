...
var (
	storagesMap    generic_sync.MapOf[string, driver.Driver]
	storagesList   []driver.Driver
	storagesListMu sync.RWMutex
)

func updateStoragesList() {
	list := storagesMap.Values()
	sort.Slice(list, func(i, j int) bool {
		if list[i].GetStorage().Order == list[j].GetStorage().Order {
			return list[i].GetStorage().MountPath < list[j].GetStorage().MountPath
		}
		return list[i].GetStorage().Order < list[j].GetStorage().Order
	})
	storagesListMu.Lock()
	storagesList = list
	storagesListMu.Unlock()
}
...
func getStorageVirtualFilesByPath(prefix string, rootCallback func(driver.Driver, model.Obj) model.Obj, filterByName string) []model.Obj {
	...
	storagesListMu.RLock()
	storages := make([]driver.Driver, len(storagesList))
	copy(storages, storagesList)
	storagesListMu.RUnlock()
