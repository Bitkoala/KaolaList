...
func MustMfa(path string) bool {
	return mustMfa(utils.FixAndCleanPath(path), true)
}

func mustMfa(path string, first bool) bool {
	meta, err := GetMetaByPath(path)
	if err == nil {
		if first && meta.Mfa {
			return true
	...
	return mustMfa(stdpath.Dir(path), false)
}
