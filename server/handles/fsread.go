...
func FsList(c *gin.Context, req *ListReq, user *model.User) {
	...
	// Kaola Vault MFA Check
	if op.MustMfa(reqPath) && !user.IsAdmin() && !common.IsMfaVerified(c, reqPath) {
		common.ErrorWithDataResp(c, errors.New("MFA required to access this vault"), 402, gin.H{
			"mfa_required": true,
			"path":         req.Path,
		})
		return
